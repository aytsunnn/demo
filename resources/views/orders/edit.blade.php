...
@section('content')
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('date_from') is-invalid @enderror"
                                       ...
                                       value="{{ old('date_from', $order->date_from) }}" required>
                                @error('date_from')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('date_to') is-invalid @enderror"
                                       ...
                                       value="{{ old('date_to', $order->date_to) }}" required>
                                @error('date_to')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select class="form-select @error('pickupPoints_id') is-invalid @enderror"
                                    ...
                                    @foreach($pickupPoints as $point)
                                        <option value="{{ $point->id }}"
                                            {{ old('pickupPoints_id', $order->pickupPoints_id) == $point->id ? 'selected' : '' }}>
                                            {{ $point->address }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="card-body">
                            ...
                            <div id="products-container">
                                @foreach($order->items as $index => $item)
                                    <div class="product-item mb-3" id="product-{{ $index }}">
                                        <div class="row">
                                            <div class="col-md-5">
                                            ...
                                            <div class="col-md-3">
                                                <input type="text" class="form-control product-price" readonly
                                                       placeholder="Цена" value="{{ $item->total_price }}">
                                            </div>
                                            <div class="col-md-1">
                                                <button type="button" class="btn btn-danger remove-product"
                                                    {{ $loop->first ? 'style=display:none;' : '' }}>×</button>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
    ...
    <script>
        ...
        document.getElementById('add-product')?.addEventListener('click', function() {
            ...
            template.id = 'product-' + productIndex;
            template.querySelectorAll('input, select').forEach(field => {
                field.name = field.name.replace(/\[\d+\]/, '[' + productIndex + ']');
            ...
            template.querySelector('.remove-product').style.display = 'inline-block';
        ...
        document.querySelectorAll('.product-item').forEach(row => updatePrice(row));
    </script>
@endsection
